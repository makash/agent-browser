...
These commands are currently available in the native Rust backend. Run them with `--native` or `AGENT_BROWSER_NATIVE=1`.
...
- `init-script` registers JavaScript with `Page.addScriptToEvaluateOnNewDocument`, so it runs before page JavaScript and persists across later navigations in the same session.
- `stealth enable` installs the built-in anti-fingerprinting preset used to reduce automation detection.
- `ssrf-protect enable` enforces strict request blocking for private and special-use destinations across navigation and sub-resource requests.
- `wait challenge cloudflare` atomically detects and waits for common Cloudflare challenges, returning a structured status instead of requiring a separate detect step.
