...
Standalone release binaries also fall back to the native daemon automatically when the packaged Node.js runtime files are not available.
...
The native daemon now includes session-scoped security and stealth primitives that are useful for scanners and long-lived automation flows:

- `init-script add|list|remove|clear` for pre-navigation JavaScript injection via CDP `Page.addScriptToEvaluateOnNewDocument`
- `stealth enable|disable` for the built-in anti-fingerprinting preset
- `ssrf-protect enable|disable` for strict blocking of private and special-use network destinations before dispatch
- `wait challenge cloudflare --timeout <ms>` for atomic challenge detection and wait

Example:

```bash
agent-browser --native stealth enable
agent-browser --native ssrf-protect enable
...
- **Default Node.js mode does not yet expose the new security preset commands**; use native mode for `init-script`, `stealth`, `ssrf-protect`, and Cloudflare challenge wait
